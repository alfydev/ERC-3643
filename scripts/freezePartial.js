...
async function main() {
  ...
  const TARGET        = "0x..."; // address to freeze
  const DECIMALS      = 18;
  const AMOUNT_HUMAN  = "5"; // freeze exactly 5 tokens
  // ==============
  ...
  if (!has("freezePartialTokens(")) {
    console.error("❌ This token does not expose freezePartialTokens(address,uint256).");
  ...
  if (has("isAgent(")) {
    ...
    if (!isAgent) {
      console.error("❌ Signer is not a token agent; cannot freeze.");
      process.exit(1);
    }
  }
  ...
  // simulate
  try {
    await token.callStatic.freezePartialTokens(TARGET, amount);
    console.log("✅ callStatic.freezePartialTokens OK – sending tx…");
  } catch (err) {
    console.error("❌ callStatic.freezePartialTokens reverted:", err.reason || err.message || err);
    process.exit(1);
  }

  // send tx
  const tx = await token.freezePartialTokens(TARGET, amount);
  console.log("⏳ freezePartialTokens tx:", tx.hash);
  ...
  console.log(`🎉 Frozen ${AMOUNT_HUMAN} tokens on ${TARGET}`);
}
